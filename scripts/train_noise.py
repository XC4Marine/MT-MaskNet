...
def train_noise(config, seed, results_path):
    """Train ResNet18 for noise type classification (single seed).
    ...
    # Load data
    train_data, val_data, test_data = load_data(config, 'noise')
    X_train, Y_train = train_data
    X_val, Y_val = val_data
    X_test, Y_test = test_data
    
    # Build and compile model
    model = build_resnet18(config['training']['input_shape'], config['training']['noise_classes'])
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=config['training']['learning_rate']),
                  loss='categorical_crossentropy',
                  metrics=['accuracy'])
    
    # Callbacks (same as sound)
    ...
    model_path = os.path.join(models_dir, f'noise_seed{seed}.keras')
    model.save(model_path)
    print(f"Saved noise model for seed {seed}: {model_path}")
    ...
    cm_path = os.path.join(figures_dir, f'noise_seed{seed}_cm.png')
    roc_path = os.path.join(figures_dir, f'noise_seed{seed}_roc.png')
    plot_cm(Y_true, Y_pred, cm_path, f'Noise CM (Seed {seed})')
    plot_roc(Y_true, Y_pred_prob, config['training']['noise_classes'], roc_path)
